...
== Manifest Templating
=== Goal
allow k8s manifests to have basic variable substitution

=== Reason
to allow basic configuration values to be defined by CI environment
for use-cases such as environment dependent ingress rules or other basic configuration
[source,yaml]
deploy:
  kubectl:
    manifests:
      - k8s-*
    values:
      Name: '{{ .POD_NAME | default "default-pod-name" }}'

== Build Arg Templating
=== Goal
allow environment variables to be passed to docker build buildArgs

=== Reason
allow configuration from the environment (such as secrets) to be used instead of hardcoded values.
[source,yaml]
build:
  artifacts:
    - imageName: gcr.io/k8s-skaffold/skaffold-example
      docker:
        buildArgs:
          message: '{{ .MESSAGE | default "default-message" }}'
          name: '{{ .NAME | required }}'


== Rollout status on deploy step
=== Goal
to be able to monitor a deployment rollout during `skaffold deploy`

=== Reason
CI pipelines should fail if a deployment fails to rollout. this should be similar to helm --wait

image::rollout-status-screenshot.png[Rollout Status Example]
